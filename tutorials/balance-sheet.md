...
Adding these assets to the [basic implementation]({{ site.url }}/tutorials/balance-sheet#basic-implementation) has reduced our capital by GBP 22.75K on the balance sheet, and corporation tax by GBP 4.3K off the P&L.
...
Here we show the Plant and Machinery accounts. The current balance reflects the write-down when we [registered fixed assets]({{ site.url }}/tutorials/balance-sheet#fixed-assets).
