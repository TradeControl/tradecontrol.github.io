---
layout: ../../layouts/Documentation.astro
title: Strategy - Overview
permalink: /strategy/strategy-overview
---

**A [schema-native](strategy-schema-native) rethink of business systems**

Trade Control is a foundational architecture for AI-native ERP. It provides a stable, extensible core from which complex workflows can be modelled, orchestrated, and scaled. Engineered with manufacturing logic at its heart, the system supports multi-dimensional connectivity—linking resources, operations, and outcomes in ways that reflect real-world processes rather than abstracted legacy constraints.

This is not a modular app stack. It’s a unified schema where declarations drive behaviour, and structure enables intelligence. From supply chains to costing, from compliance to scheduling, Trade Control offers a platform where business logic is expressed natively and optimised continuously.

Accounting is our initial domain of deployment—not our defining purpose. The architecture is designed to support recursive operations, transactional clarity, and embedded optimisation across all business functions.

This strategy document outlines the schema, logic, and publishing roadmap.

**Provenance Note**  
> This Strategy draws on foundational material published in the [*Production Systems*](/articles/tc_intro) series. The original work spans from the [Theory of Production](/articles/tc_production), first published on 25 November 2019, to the [final article](/articles/tc_tax) dated 15 August 2021. At that point, the project was paused and the author transitioned to production systems development in the West Country. Development recommenced October 2025.

## Section 1 What's it all about?

### A. Functional Description

- [Function](/strategy/strategy-section-one-a-function)

### B. Conceptual Description

- [Money (Accounts)](/strategy/strategy-section-one-b-concept-money)
- [Assets (Markets)](/strategy/strategy-section-one-b-concept-assets)
- [Commerce](/strategy/strategy-section-one-b-concept-commerce)

### C. Operational Implementation

- [Cash](/strategy/strategy-section-one-c-operations-cash)
- [Supply versus Demand](/strategy/strategy-section-one-c-operations-svd)
- [Tax](/strategy/strategy-section-one-c-operations-tax)

## Section 2 Mission Objectives

- [Objectives](/strategy/strategy-section-two-mission)
- [Intelligence Test](/strategy/strategy-intelligence-test)
- [Example Application](/strategy/strategy-section-two-example)

*This overview is the first step in a recursive journey—where declarations shape outcomes, and structure becomes strategy.*

## Licence

![Creative Commons](https://i.creativecommons.org/l/by-nd/4.0/88x31.png)

Licenced by Trade Control Ltd under a [Creative Commons Attribution-NoDerivatives 4.0 International License](http://creativecommons.org/licenses/by-nd/4.0/)
